...
def main():
  ...
  test_out = np.concatenate((np.expand_dims(np.array(range(4220),dtype=np.int), axis=1), np.argmax(pred,axis=1)[:,np.newaxis]), axis=1)
  header = np.array([["id", "digit"]])
  test_out = np.concatenate((header, test_out))
  np.savetxt('test_predicted3.csv', test_out, fmt='%s', delimiter=',')
...
class LinearLayer:
  ...
  def backward(self, grad):
    ...
    for i in grad:
      total += sum(i)
    self.grad_weights = np.matmul(np.transpose(self.input), grad)
    self.grad_bias = total
    return np.matmul(grad, np.transpose(self.weights))
